  ...
  const handleRegisterSubmit = async (ele) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Registration Failed",
        text: "Error while registering. Please try again later.",
      });
    }
  };
  ...
  const handleOtpSubmit = async (ele) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/register/verifyOtp`, {
        method: "POST",
        body: JSON.stringify({ email: data.email, ...otpData, adminValidation }),
        headers: {
          "Content-Type": "application/json",
        },
      });
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Error while verifying OTP. Please try again later.",
      });
    }
  };
          ...
          <div className="col-md-6">
            <div className="card" style={{ border: '2px solid black', borderRadius: '15px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)', backgroundColor: 'rgba(0, 0, 0, 0.7)', marginLeft: '110px', marginRight: '50px' }}>
              <div className="card-body">
                <h1 className="text-center mb-4" style={{ color: '#FFA500', fontFamily: 'times roman' }}>Welcome To MyBus</h1>
                  ...
                  <form onSubmit={handleOtpSubmit}>
                    ...
                    <div className="mb-3">
                      ...
                      >
                        ...
                        <option value="admin">Bus Administrator</option>
                      </select>
                    </div>
                    {otpData.role === "admin" && (
                      <div className="mb-3">
                        <label htmlFor="adminValidation" className="form-label" style={{ color: '#FFA500' }}>
                        ...
                        <input
                          ...
                          onChange={(e) => setAdminValidation(e.target.value)}
